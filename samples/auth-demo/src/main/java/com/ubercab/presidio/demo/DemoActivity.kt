...
fun AuthScreen(
  ...
  var selectedEnv by remember { mutableStateOf(UriConfig.UberEnvironment.PRODUCTION) }
  val envOptions = UriConfig.UberEnvironment.entries

  Column(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp)) {
    ExposedDropdownMenuBox(expanded = envExpanded, onExpandedChange = { envExpanded = !envExpanded }) {
      Box(
        modifier = Modifier.fillMaxWidth().clickable { envExpanded = true }.menuAnchor().padding(16.dp)
      ) {
        TextField(
...
fun AuthButton(
  ...
  launcher: ActivityResultLauncher<Intent>,
  authDestination: AuthDestination,
  environment: UriConfig.UberEnvironment,
  buttonText: String,
    ...
    onClick = {
      UberAuthClientImpl()
        .authenticate(
          ...
          launcher,
          AuthContext(authDestination, AuthType.PKCE(), prefillInfo, environment = environment),
        )
    },
  )
}
